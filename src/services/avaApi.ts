...
class AvaApiService {
  ...
  /**
   * Send voice chat message to AVA
   * Uploads audio file and gets AI response
   */
  async voiceChat(
    audioBlob: Blob,
    conversationId?: string,
    ...
    try {
      ...
      const response = await fetch(`${this.baseUrl}/ava/voice`, {
        method: 'POST',
        body: formData,
      })
  ...
  /**
   * Get voice greeting from AVA
   * Converts text greeting to speech using AWS Polly
   */
  async helloVoice(userId: string = 'anonymous', content: string): Promise<string> {
    try {
      const response = await fetch(`${this.baseUrl}/ava/hello/voice`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      console.error('Hello voice API error:', error)
      throw error
    }
  }
}
